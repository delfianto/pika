    ...
    match req.method.as_str() {
        "scan.start" | "scan.filter" | "scan.aob" | "pointer.scan" => {
            ...
            .await
            {
                ...
                Ok(Err(resp)) => resp,
                Err(e) => JsonRpcResponse::err(id, INTERNAL_ERROR, format!("task panicked: {e}")),
            };
        }
        _ => {}
    }
...
fn dispatch_heavy(state: &RpcState, method: &str, params: &serde_json::Value) -> MethodResult {
    match method {
...
fn handle_pid_list() -> MethodResult {
    let processes = pid::list_wine_processes()
...
fn handle_pid_find(params: &serde_json::Value) -> MethodResult {
    ...
    let processes = pid::find_process(name)
        .map_err(|e| JsonRpcResponse::err(None, INTERNAL_ERROR, e.to_string()))?;
    Ok(json!(processes))
}
...
fn handle_scan_start(state: &RpcState, params: &serde_json::Value) -> MethodResult {
    ...
    let session = first_scan(state.mem.as_ref(), pid, value, dtype)
        .map_err(|e| JsonRpcResponse::err(None, INTERNAL_ERROR, e.to_string()))?;
...
fn handle_scan_filter(state: &RpcState, params: &serde_json::Value) -> MethodResult {
    ...
    let mode_str = params
        ...
        .and_then(|v| v.as_str())
        .unwrap_or("exact");
    let mode = crate::scan::filter::FilterMode::from_str_loose(mode_str)
    ...
    let result = state.sessions.with_session(session_id, |session| {
        ...
        let top_candidates: Vec<CandidateJson> = session
            .candidates
            .iter()
            .take(100)
            .map(|c| c.into())
...
fn handle_scan_aob(state: &RpcState, params: &serde_json::Value) -> MethodResult {
    ...
    let include_readonly = params
        .get("include_readonly")
    ...
    let addresses =
        crate::scan::engine::aob_scan(state.mem.as_ref(), pid, &pattern, include_readonly)
            .map_err(|e| JsonRpcResponse::err(None, INTERNAL_ERROR, e.to_string()))?;
...
fn handle_memory_read(state: &RpcState, params: &serde_json::Value) -> MethodResult {
    ...
    let address = parse_hex_address(params.get("address")).ok_or_else(|| {
        JsonRpcResponse::err(None, INVALID_PARAMS, "missing or invalid 'address'")
    })?;
...
fn handle_memory_write(state: &RpcState, params: &serde_json::Value) -> MethodResult {
    ...
    write_value(state.mem.as_ref(), pid, address, value, dtype)
        .map_err(|e| JsonRpcResponse::err(None, INTERNAL_ERROR, e.to_string()))?;
    ...
    let result = state.sessions.with_session(session_id, |session| {
        ...
        for candidate in &session.candidates {
            ...
            match write_value(state.mem.as_ref(), session.pid, addr, value, dtype) {
                ...
                Err(e) => {
                    errors.push(json!({"address": format!("{addr:#x}"), "error": e.to_string()}));
                }
            }
        }
...
fn handle_freeze_start_all(state: &RpcState, params: &serde_json::Value) -> MethodResult {
    ...
    let pid = state
        .sessions
        .with_session(session_id, |s| s.pid)
        .unwrap_or(0);
...
mod tests {
    ...
    fn parse_hex_addresses() {
        ...
        assert_eq!(
            parse_hex_address(Some(&json!("0X140000000"))),
            Some(0x0001_4000_0000)
        );
        assert_eq!(
            parse_hex_address(Some(&json!("DEADBEEF"))),
            Some(0xDEAD_BEEF)
        );
